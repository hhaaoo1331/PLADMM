function [U,PSNR,SNR,SSIM,i,f,t] = NonTVADMM_Arctan(I,F,H,maxit,mu,s,beta1,beta2,rho,gamma)
...
%    
% f(t) = 2/(s*sqrt(3))*(atan((1+2*s*abs(t))/sqrt(3))-pi/6)
%=============================================================
...
for i = 1 : maxit
     ...
     Arctan_c = 1;
     X = max(abs(KUF + Lam1/beta1) - (mu*Arctan_c)/beta1, 0).*sign(KUF + Lam1/beta1);
...
function [D,Dt] = defDDt
 ...
 function [f] = fval(D1U,D2U,KUF,s,mu)
       ...
       fid = 2/(s*sqrt(3))*(atan((1+2*s*abs(KUF))/sqrt(3))-pi/6);
       fid = sum(sum(fid));
       f = tv + mu*fid;
 return;
