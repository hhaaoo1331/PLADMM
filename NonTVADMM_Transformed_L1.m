function [U,PSNR,SNR,SSIM,i,f,t] = NonTVADMM_Transformed_L1(I,F,H,maxit,mu,s,beta1,beta2,rho,gamma)
...
%    
% f(t) = (s+1)/s*abs(t)-((1+s)*abs(t))./(abs(t)+a);
%=============================================================
...
for i = 1 : maxit
     ...
     df = gradient_Transformed_L1(KUF,s);
     ...
     Transformed_L1_c = (s+1)/(s);
     X = max(abs(KUF + Lam1/beta1) - (mu*Transformed_L1_c)/beta1, 0).*sign(KUF + Lam1/beta1);
...
function [D,Dt] = defDDt
 ...
 function [f] = fval(D1U,D2U,KUF,s,mu)
       ...
       fid = (s+1)/s*abs(KUF)-((1+s)*abs(KUF))./(abs(KUF)+a);
       fid = sum(sum(fid));
       f = tv + mu*fid;
 return;
