function [U,PSNR,SNR,SSIM,i,f,t] = NonTVADMM_SCAD(I,F,H,maxit,mu,s1,s2,beta1,beta2,rho,gamma)
...
%             
%        ------ abs(t)                               if abs(t)<=s1
% f(t) = ------(-t.^2+2*s2*abs(t)-s1^2)/(2*(s2-s1))  if s1<=abs(t)<=s2
%        ------ (s1+s2)/2                            if abs(t)>s2
...
% s1,s2 ------------------------  Parameters in non-convex functions
...
for i = 1 : maxit
     ...
     SCAD_c = 1;
     X = max(abs(KUF + Lam1/beta1) - (mu*SCAD_c)/beta1, 0).*sign(KUF + Lam1/beta1);
...
function [D,Dt] = defDDt
 ...
 function [f] = fval(D1U,D2U,KUF,s1,s2,mu)
       tv = sum(sum(sqrt(D1U.^2 + D2U.^2)));
       fid = SCAD_function(KUF,s1,s2);
       fid = sum(sum(fid));
       f = tv + mu*fid;
 return;
